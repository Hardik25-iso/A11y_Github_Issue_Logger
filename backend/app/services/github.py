...
async def search_issues(issue: ScanIssue, repo: str, token: str | None = None) -> tuple[list[SimilarIssue], str, str]:
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            response = await client.get(f"{GITHUB_API}/search/issues", params=params, headers=_headers(token))
...
def markdown_body(issue: GeneratedIssue, screenshot_url: str | None = None) -> str:
    steps = "\n".join(f"{index}. {step}" for index, step in enumerate(issue.repro_steps, 1))
    criteria = "\n".join(f"- [ ] {item}" for item in issue.acceptance_criteria)
    screenshot_section = (
        f"\n## Screenshot\n![Captured element from the live accessibility scan]({screenshot_url})\n"
        if screenshot_url
        else ""
    )
...
async def _upload_screenshot(repo: str, data_uri: str, token: str | None) -> str | None:
    """Commit a screenshot to the target repo and return its raw URL for
    embedding. Returns None on any failure (e.g. token lacks Contents:write)
    so issue creation can proceed without the image."""
    try:
        b64 = data_uri.split(",", 1)[1]
        # Validate it decodes; reject anything malformed before committing.
        base64.b64decode(b64, validate=True)
    except (IndexError, ValueError):
        return None

    path = f".a11y-screenshots/{uuid.uuid4().hex}.png"
    payload = {"message": "Add accessibility scan screenshot", "content": b64}
    try:
        ...
        if response.status_code >= 400:
            log_warning(f"Screenshot upload skipped: GitHub returned {response.status_code}")
            return None
        return response.json().get("content", {}).get("download_url")
    except httpx.HTTPError as exc:
        log_warning(f"Screenshot upload skipped: {type(exc).__name__}")
        return None


async def create_issue(
    repo: str, issue: GeneratedIssue, token: str | None = None, screenshot: str | None = None
) -> dict:
    effective_token = token or get_settings().github_token
    if not effective_token:
        raise HTTPException(
            status_code=503,
            detail="A GitHub token is required to log an issue. Provide your own PAT in the request or configure GITHUB_TOKEN on the server.",
        )
    screenshot_url = await _upload_screenshot(repo, screenshot, token) if screenshot else None
    payload = {
        "title": issue.title,
        "body": markdown_body(issue, screenshot_url),
        "labels": issue.labels,
        "assignees": [issue.assignee] if issue.assignee else [],
    }
    async with httpx.AsyncClient(timeout=20) as client:
        response = await client.post(f"{GITHUB_API}/repos/{repo}/issues", json=payload, headers=_headers(token))
    if response.status_code >= 400:
